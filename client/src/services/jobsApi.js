...
// WARNING: adapte BASE_URL et les paramètres de requête selon ta config réelle

const BASE_URL = import.meta.env.VITE_API_URL;

// récupère une page de la liste des offres (route listing)
export async function fetchJobOffers({ page = 0 } = {}) {
  const res = await fetch(`${BASE_URL}/job-offers?page=${page}`);
  if (!res.ok) {
    throw new Error("Erreur lors de la récupération des offres");
  }
  return res.json(); // { job_offers: [...], is_the_end: boolean }
}

// récupère le détail d'une offre précise (route offre)
