...
const STATUS_CONFIG = {
    pending: { label: "En attente", style: "bg-primary-light/20 text-font-primary-dark", Icon: Clock },
    ...
    return (
        <li className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 py-4 border-b border-primary-light/30 last:border-none max-h-135 overflow-auto scrollbar-thumb-primary">
            <div className="min-w-0">
                <p className="font-semibold text-sm text-font-primary-dark">{applicant.name}</p>
                ...
                <p className="text-xs mt-0.5 text-font-primary-dark/50">
    ...
    if (!job) {
        return (
            <div className="bg-bone-light rounded-2xl shadow-[0_0_15px_rgba(0,0,0,0.08)] border border-primary-light/40 p-10 flex flex-col items-center text-center gap-3">
                <Users size={28} className="text-font-primary-dark/40" aria-hidden="true" />
                <p className="font-semibold text-font-primary-dark">Sélectionnez une offre</p>
    ...
    return (
        <article className="bg-bone-light rounded-2xl shadow-[0_0_15px_rgba(0,0,0,0.08)] border border-primary-light/40 p-5 sm:p-7">
            <div className="flex items-center gap-2 mb-1">
                ...
                <h3 className="font-bold text-font-primary-dark">
                    Candidatures — {job.title}
                </h3>
            </div>
            <p className="text-sm text-font-primary-dark/60 mb-4">
                {job.applicants.length} candidature{job.applicants.length > 1 ? "s" : ""} reçue{job.applicants.length > 1 ? "s" : ""}
            </p>

            {job.applicants.length === 0 ? (
                <p className="text-sm text-center py-8 text-font-primary-dark/60">
