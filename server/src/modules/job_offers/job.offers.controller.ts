...
const getJobOffers = async (req: Request, res: Response, next: NextFunction) => { 
    try {
        ...
        const [rows] = await pool.query(
            "SELECT PK_content_hash, title, contract_type, city, country, company FROM Job_Offers ORDER BY publish_date DESC LIMIT ? OFFSET ?;", 
            [limit + 1, offset]
        );
...
const getJobOfferById = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const [rows] = await pool.execute<any[]>(
            "SELECT PK_content_hash, title, description, url, contract_type, city, country, company, is_remote_job, is_hybride_job, publish_date, salary_max, salary_min, currency FROM Job_Offers WHERE PK_content_hash = ?;",
            [id]
        );
